// ─────────────────────────────────────────────────────────────
// lib/cost.js — 예산 판단과 비용 추정
//
// 두 가지 일을 한다.
//   1) 스코어링 보정 — 선택한 예산대(저예산·보통·프리미엄)와 장소의
//      비용대를 대조해 점수를 올리거나 내린다 (budgetAdjust)
//   2) 금액 추정   — 하루 코스의 예상 비용을 입장·식사·이동으로 나눠 계산한다
//      (estimateDayCost) — 실제 요금 데이터가 없을 때 쓰는 눈대중용 추정치다
//
// 실제 데이터(입장료 문구·Google priceLevel)가 있으면 항상 그걸 우선하고,
// 없을 때만 장소 종류로 추정한다.
//
// 쓰는 곳: lib/course.js (점수 보정) · CourseDetail (예상 비용 표시)
// ─────────────────────────────────────────────────────────────
