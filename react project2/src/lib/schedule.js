...
// 자연·전망 같은 야외 경관 위주 장소는 영업시간 데이터가 아예 없는 경우가 많아 hasHours
// 체크로는 못 거른다. 대신 초저녁(19:00)을 넘겨 도착하면 어두워서 제대로 보기 어렵다고 보고,
// 마감처럼 취급한다(마감이 있는 곳은 여전히 그 시각을 우선한다 — 아래 hasHours 분기 참고).
const DUSK_CUTOFF_MIN = 19 * 60
const DAYLIGHT_ONLY_KINDS = new Set(['nature', 'viewpoint'])
...
export function scheduleDay(places, { dayStartMin = DEFAULT_DAY_START_MIN, transport = '대중교통' } = {}) {
  ...
  return places.map((place, index) => {
    ...
    if (!hoursNote && DAYLIGHT_ONLY_KINDS.has(placeKindOf(place)) && arriveMin >= DUSK_CUTOFF_MIN) {
      hoursNote = `도착 예정 ${formatClock(arriveMin)}엔 이미 어두워서 야외 경관을 보기 어려워요`
    }
...
// 자연·전망처럼 초저녁 이후엔 사실상 못 보는 곳도 같은 취급 — course.js 가 이 함수로 "이 슬롯,
// 다른 곳으로 바꿀 수 있으면 바꾼다"를 판단하기 때문에, 마감과 동일한 신호를 줘야 교체가 일어난다.
export function arrivesAfterClose(place) {
  if (!place) return false
  if (DAYLIGHT_ONLY_KINDS.has(placeKindOf(place)) && place.arriveMin >= DUSK_CUTOFF_MIN) return true
  if (place.alwaysOpen) return false
...
export function isOpenAround(place, minute) {
  ...
  if (DAYLIGHT_ONLY_KINDS.has(placeKindOf(place)) && minute >= DUSK_CUTOFF_MIN) return false
