// ─────────────────────────────────────────────────────────────
// data/travelOptions.js — 사용자가 고르는 조건들의 목록
//
// 테마·기간·예산·교통편·스타일처럼 화면에 버튼으로 뿌려지는 선택지를 한곳에 모았다.
// 여기 값이 그대로 코스 생성 로직(lib/course.js)의 입력이 되므로,
// 라벨 문자열이 곧 키 역할을 한다 — 라벨을 바꾸면 스코어링 조건도 같이 봐야 한다.
//
// SLOT_LABELS(오전·점심 맛집·오후 카페·저녁)가 특히 중요하다.
// 하루 코스의 뼈대이자 장소를 배치하는 칸 이름이다.
// ─────────────────────────────────────────────────────────────
