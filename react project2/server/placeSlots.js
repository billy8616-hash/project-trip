// ─────────────────────────────────────────────────────────────
// server/placeSlots.js — 장소를 어느 시간대에 넣을지 정하기
//
// "이 장소는 오전에 어울리는가, 저녁에 어울리는가"를 외부 API 의 분류 코드로 판단한다.
// 코스의 네 칸(오전·점심 맛집·오후 카페·저녁)을 채우는 기준이 되는 정보다.
//
// 한 장소가 여러 시간대에 어울릴 수 있어서 배열로 돌려준다 —
// 공원은 오전·오후 둘 다, 식당은 점심·저녁 둘 다 가능하다.
//
// 여기서 쓰는 라벨은 src/data/travelOptions.js 의 SLOT_LABELS 와 반드시 같아야 한다.
// 서버가 붙인 슬롯을 프런트가 그대로 비교하기 때문이다.
// ─────────────────────────────────────────────────────────────
