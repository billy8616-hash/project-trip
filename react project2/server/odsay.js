// ─────────────────────────────────────────────────────────────
// server/odsay.js — ODsay 대중교통 길찾기
//
// 대중교통 모드에서 지도에 그릴 "실제 지하철·버스 노선"을 받아 온다.
// 직선으로 이은 선과 달리 몇 호선을 타는지까지 나오므로 배지에 노선명을 표시할 수 있다.
//
// 하루 1,000회 무료 한도가 있어서 메모리 캐시를 둔다. 새로고침하거나 교통편을
// 바꿨다 되돌릴 때 같은 구간을 다시 부르지 않게 하려는 것이다.
// 좌표를 소수점 4자리(약 11m)로 끊어 캐시 키를 만들어, 미세하게 다른 좌표도 같은 구간으로 본다.
//
// 경로를 못 찾는 구간(시골 등)은 null 로 돌려주고, 화면 쪽이 그 구간만 직선으로 그린다.
// ─────────────────────────────────────────────────────────────
