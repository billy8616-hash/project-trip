// ─────────────────────────────────────────────────────────────
// server/openingHours.js — 자유 문장으로 된 영업시간을 숫자로 바꾸기
//
// TourAPI 의 영업시간은 정해진 형식이 없는 한국어 문장이다.
//   "09:00~18:00" · "[하절기] 09:00~19:00 [동절기] 09:00~18:00"
//   "매주 월요일 휴관" · "연중무휴" · "동절기 1시간 단축"
// 이걸 코스 로직이 쓸 수 있는 숫자(자정 기준 분)와 요일 배열로 바꾼다.
//
// 설계에서 가장 중요한 판단: 애매하면 포기한다.
// 억지로 해석해서 틀린 값을 만들면, 멀쩡히 열려 있는 장소가 코스에서 빠지거나
// 닫힌 곳에 사용자를 보내게 된다. 확실히 읽히는 것만 구조화하고 나머지는
// null·빈 배열로 남겨 "정보 없음"으로 취급한다.
//
// 원문은 따로 저장해 화면에 그대로 보여 준다 — 파싱은 판단용, 원문은 표시용.
// ─────────────────────────────────────────────────────────────
